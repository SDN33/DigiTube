...
interface ModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (details: { url: string; email: string; likes: string; amount: string }) => Promise<void>;
  selectedLikes: string;
  selectedPrice: string;
}

const ModalContent = ({ isOpen, onClose, onSubmit, selectedLikes, selectedPrice }: ModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-50 w-full max-w-md bg-white rounded-lg shadow-xl p-6 m-4">
        <h2 className="text-2xl mb-6 font-bold text-center text-gray-800">Commander {selectedLikes} likes</h2>
        <div className="bg-black p-2 rounded-lg text-white">
          <p className="text-center">
            <span className="block text-lg font-semibold mt-4">Pack sélectionné</span>
            <span className="text-2xl font-bold">{selectedLikes} likes</span>
